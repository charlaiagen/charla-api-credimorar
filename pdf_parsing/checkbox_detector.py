...
class CheckboxDetector:
    ...
    def detect_checkboxes(self, image_array, output_dir="output/debug_images"):
        """
        Detect checkboxes in an image and create debug visualizations.
        ...
        Returns:
            tuple: (checkboxes list, debug image path)
        """
        ...
        threshold_path = output_path / f"threshold_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
        cv2.imwrite(str(threshold_path), thresh)
        ...
        # Draw all contours in blue (before filtering)
        ...
        for i, contour in enumerate(contours):
            result = self._analyze_contour(contour, gray, thresh, i, debug_image)
            if result:
                checkboxes.append(result) if result.get('is_checkbox') else rejected_contours.append(result)
        
        # Log detection statistics
        logging.info(f"Detected {len(checkboxes)} checkboxes")
        logging.info(f"Rejected {len(rejected_contours)} contours")
        
        # Draw rejected contours in red with reason
        self._draw_rejected_contours(debug_image, rejected_contours)
        
        # Save debug image
        debug_path = output_path / f"debug_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
        cv2.imwrite(str(debug_path), debug_image)
        
        return checkboxes, debug_path
    
    def _analyze_contour(self, contour, gray_image, thresh_image, index, debug_image):
        ...
        # Draw visualization
        color = (0, 255, 0) if is_checked else (0, 255, 255)
        cv2.rectangle(debug_image, (x, y), (x+w, y+h), color, 2)
        label = f"#{index} {'✓' if is_checked else '☐'} ({std_value:.1f})"
    ...
    def _draw_rejected_contours(self, debug_image, rejected_contours):
        """Draw rejected contours on the debug image."""
        for i, rejected in enumerate(rejected_contours):
            if rejected.get('position'):
                x, y, w, h = rejected['position']
                cv2.rectangle(debug_image, (x, y), (x+w, y+h), (0, 0, 255), 1)
                reason = rejected['reason'][:3]
                cv2.putText(debug_image, f"R{i}:{reason}", (x, y-5), 
